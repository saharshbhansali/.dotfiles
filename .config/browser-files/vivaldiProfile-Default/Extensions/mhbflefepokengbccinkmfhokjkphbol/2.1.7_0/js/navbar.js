const nav_bar_change = () => {
  ...
  span.innerHTML = `
    ...
    `;
  if (time_table) span.innerHTML += `<button class="btn btn-primary border-primary shadow-none" type="button" style="background: rgba(13,110,253,0);border-style: none;" onclick="document.getElementsByClassName('dropdown-item menuFontStyle systemBtnMenu')[${time_table}].click()" id="nav_short">Time Table</button>`;
  else span.innerHTML += `<button class="btn btn-primary border-primary shadow-none" type="button" style="background: rgba(13,110,253,0);border-style: none;" onclick="document.getElementsByClassName('dropdown-item menuFontStyle systemBtnMenu')[${calendar}].click()" id="nav_short">Calendar</button>`;
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.message === "nav_bar_change") {
    try {
      ...
      if (document.getElementsByClassName("btn btn-primary border-primary shadow-none").length == 0 && flag) {
        nav_bar_change();
      }
    } catch (error) {
      console.log(error);
